...
def abrir_tela_alterar_usuario(id_usuario, callback_atualizar=None):
    ...
    tk.Label(aba_cadastro, text="Senha nova:").grid(row=3, column=0, sticky="w", padx=10, pady=5)
    entry_senha_nova = tk.Entry(aba_cadastro, show="*", width=40)
    entry_senha_nova.grid(row=3, column=1, padx=10, pady=5)

    tk.Label(aba_cadastro, text="E-mail:").grid(row=4, column=0, sticky="w", padx=10, pady=5)
    entry_email = tk.Entry(aba_cadastro, width=40)
    if email_db:
        entry_email.insert(0, email_db)
    entry_email.grid(row=4, column=1, padx=10, pady=5)

    tk.Label(aba_cadastro, text="Nível de Acesso:").grid(row=5, column=0, sticky="w", padx=10, pady=5)
    ...
    combo_nivel.grid(row=5, column=1, padx=10, pady=5)

    # Checkbox para marcar se o usuário está ativo
    ativo_var = tk.BooleanVar(value=ativo_db)
    tk.Checkbutton(aba_cadastro, text="Ativo", variable=ativo_var).grid(row=6, column=1, sticky="w", padx=10, pady=5)
    ...
    def salvar_alteracoes():
        ...
        if senha != senha_nova:
            messagebox.showwarning("Aviso", "Senha e confirmar senha estão diferentes!")
            return
